...
bool CDescription::ReadFromBuffer(char **pBuffer)
{
    *pBuffer = engine->COM_ParseFile(*pBuffer, token);
    ...
    {
        Msg(_T("Expecting 'VERSION', got '%s'"), token);
        return false;
    }
    ...
    *pBuffer = engine->COM_ParseFile(*pBuffer, token);
    ...
    {
        Msg(_T("Version mismatch, expecting %f, got %f"), SCRIPT_VERSION, fVer);
        return false;
    }
    ...
    *pBuffer = engine->COM_ParseFile(*pBuffer, token);
    ...
    {
        Msg(_T("Expecting 'DESCRIPTION', got '%s'"), token);
        return false;
    }
    ...
    *pBuffer = engine->COM_ParseFile(*pBuffer, token);
    ...
    if (strlen(token) <= 0)
    {
        Msg(_T("Expecting '%s'"), m_pszDescriptionType);
        return false;
    }

    if (stricmp(token, m_pszDescriptionType))
    {
        Msg(_T("Expecting %s, got %s"), m_pszDescriptionType, token);
        return false;
    }
    ...
    while (1)
    {
        ...
        *pBuffer = pStart;
        ...
        else
        {
            ...
            if (!pObj)
            {
                Msg(_T("Couldn't create script object"));
                return false;
            }
