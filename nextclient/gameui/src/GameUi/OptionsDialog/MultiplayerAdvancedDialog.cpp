...
const char kLegacyKillFeedCvar[] = "hud_deathnotice_old";

namespace vgui2 {
    class ComboBox;
}

CMultiplayerAdvancedDialog::CMultiplayerAdvancedDialog(vgui2::Panel *parent) :
    BaseClass(parent, "MultiplayerAdvancedDialog")
{
    ...
    if (!m_pDescription->FindObject(kLegacyKillFeedCvar))
        m_pDescription->AddObject(CreateLegacyKillFeedScriptObj());
...
CScriptObject* CMultiplayerAdvancedDialog::CreateLegacyKillFeedScriptObj()
{
    auto legacy_kill_feed = new CScriptObject();
    legacy_kill_feed->type = O_BOOL;
    V_strcpy_safe(legacy_kill_feed->prompt, "#Cstrike_LegacyKillFeed");
    V_strcpy_safe(legacy_kill_feed->cvarname, kLegacyKillFeedCvar);

    return legacy_kill_feed;
}
