...
namespace
{
    ...
    constexpr char kCvarsPathId[] = "GAMECONFIG";
    ...
    {
        if (!g_CvarsSandboxAvailable || cls->state != ca_active || !g_SandboxedCvars.contains(name))
        {
    ...
    int MsgFunc_SandboxCvar(const char* pszName, int iSize, void* pbuf)
    {
        if (!g_CvarsSandboxAvailable)
            return 1;
        ...
        for (int i = 0; i < cvars_count; i++)
        {
            ...
            bool has_illegal_symbols = std::find_if(msg_cvar_value.begin(), msg_cvar_value.end(), [](char ch) {
                                           return ch == '\n' || ch == ';';
                                       }) != msg_cvar_value.end();
            ...
            else
            {
                ...
                Con_DPrintf(
                    ConLogType::InfoColored,
                    "The server has set the cvar \"%s\" to \"%s\" until you disconnect\n",
                    cvar_name.c_str(),
                    msg_cvar_value.c_str()
                );
            }
        }
