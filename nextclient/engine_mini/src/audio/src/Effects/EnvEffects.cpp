...
namespace MetaAudio
{
    ...
    GainFading& value)
  {
    ...
    auto result = fader->ToNewValue(
      ...
      value.target,
      static_cast<float>(cl->time - cl->oldtime)
      );
  ...
  void EnvEffects::ApplyEffect(aud_channel_t* ch, qboolean underwater)
  {
    ...
    if (ch->entnum != cl->viewentity && pent != nullptr && sent != nullptr)
    {
  ...
  {
    char* _al_maxsends;
    gEngfuncs.CheckParm("-al_maxsends", &_al_maxsends);
  ...
  void EnvEffects::OverrideEffects()
  {
    ...
    FS_GetCurrentDirectory(directory.data(), directory.size());
